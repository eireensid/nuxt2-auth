<template>
  <div class="d-flex flex-column w-100 p-5">
    ...
    >
      ...
      <p class="w-50 mt-3 danger" v-show="authError.show">
        {{ authError.text }}
      </p>
      <div class="w-50" :class="authError.show ? 'mt-2' : 'mt-4'">
        <BButton type="submit" variant="success" :disabled="btnDisabled"
          >Войти</BButton
        >
      </div>
    </BForm>
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          if (content.access_token) {
            this.saveTokenToLocal(content.access_token)
            this.$nextTick(() => {
